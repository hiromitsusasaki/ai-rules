...
個人用 AI エージェントルールを `atoms/*.yaml` で一元管理（SSOT）し、複数の AI ツール向けにルール文書を生成・配布する Gemini CLI。SSOT は自動改変せず、必ず提案→手動承認のフローを経る設計。

## コマンド一覧

```bash
# Gemini CLI 実行（npm scripts または直接）
node ./bin/gemini <command>

# 主要コマンド
node ./bin/gemini ingest <url>          # URL からルール素材を取り込み
node ./bin/gemini ingest --text "..."   # テキストから取り込み
node ./bin/gemini ingest --stdin        # stdin から取り込み
node ./bin/gemini ingest --file <path>  # ファイルから取り込み
node ./bin/gemini propose --last        # 最新 inbox から提案生成
node ./bin/gemini approve --last        # 最新提案を承認（project モード）
node ./bin/gemini build                 # atoms → dist 生成
node ./bin/gemini install               # dist → ホーム配下に同期
node ./bin/gemini configure             # API キー設定
node ./bin/gemini doctor                # YAML スキーマ・リンク切れチェック
node ./bin/gemini status                # 未レビュー提案・期限超過表示
```
...
5. **Build**: `atoms/*.yaml` から Gemini CLI 向けに `dist/<target>/*` をレンダリング
6. **Install**: dist ファイルを symlink またはコピーでホーム配下に同期

### 動作モード

- **Global モード**: `~/.config/gemini/` をルートとして atoms ベースで動作
- **Project モード**: カレントディレクトリに `.gemini/` がある場合に起動。`project.md` をソースとし、ターゲット別に生成。必要に応じて最適化処理を行う
