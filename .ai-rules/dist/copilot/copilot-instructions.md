...
本ルール文書は GitHub Copilot 向けに最適化されています。`.github/copilot-instructions.md` として配置してください。
...
- GitHub Copilot 用のカスタム指示書は `dist/chat/custom_instructions.txt` に出力されます。
- must/should のルールを中心に、Copilot の補完品質向上に寄与する指示を含みます。
- ルールは簡潔かつ明確に記述し、Copilot のコンテキスト理解を助ける形式を推奨します。
